...
# Optionally select a compiler launcher. This is intended for compiler caching
# tools (e.g. ccache) but can be used for anything that can call a compiler.
#
...
# Bocfel’s support for version 6 is incomplete. It mostly works well enough in
# graphics-supporting Glk implementations, if you restrict yourself to Infocom’s
# games. It will likely never properly support arbitrary V6 games unless those
# games restrict themselves to features that are already in version 5/7/8. If
# this variable is defined (with any value), then version 6 support will not be
# built. This results in a measurably smaller binary.
#
# NO_V6=		1

# Bocfel includes a lot of code to handle user options (via the command-line, a
# file, or environment variables). There are some environments where this is
# entirely unnecessary—where the user has no ability to control options, for
# example—which results in large amounts of effectively dead code. If the
# following variable is defined (with any value), most of the options-handling
# code will not be compiled in.
#
# NO_OPTIONS=		1

# The Zoom interpreter added 4 new EXT opcodes used for interpreter
# benchmarking. Bocfel includes support for these, but it is an extremely niche
# use-case which most users won’t ever need, so these are disabled by default,
# but can be re-enabled at compile-time. If this variable is set (with any
# value), then the Zoom opcodes will be enabled.
#
# ZOOM_OPCODES=		1
